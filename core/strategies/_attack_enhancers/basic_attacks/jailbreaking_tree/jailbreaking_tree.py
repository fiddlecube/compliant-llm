...
class JailbreakingTree(AttackEnhancement):
    ...
    def enhance(self, attack: str) -> AttackEnhancementResult:
        ...
        return AttackEnhancementResult(
            best_node.prompt, "text", metadata={"best_node": best_node}
        )
    ...
    def expand_node(
        ...
        for _ in range(num_branches):
            ...
            improvement_prompt: ImprovementPrompt = self._generate_schema(
                conversation, ImprovementPrompt
            )
            enhanced_attack = improvement_prompt.prompt
            ...
            non_refusal: NonRefusal = self._generate_schema(
                non_refusal_prompt, NonRefusal
            )
            classification = non_refusal.classification
            ...
            rating: Rating = self._generate_schema(judge_prompt, Rating)
            score = rating.rating
    ...
    async def a_enhance(
        ...
        return AttackEnhancementResult(
            output_text=best_node.prompt,
            output_type="text",
            metadata={
                "best_node": best_node,
                "attack_enhancement": "jailbreaking_tree",
            },
        )
    ...
    async def a_generate_child(
        ...
        target_response = await self.target_model.agenerate(enhanced_attack)
